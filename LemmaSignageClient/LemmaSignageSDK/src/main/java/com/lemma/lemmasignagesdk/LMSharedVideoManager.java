...
public class LMSharedVideoManager implements AdQueueManager.AdQueueManagerListener, LMSharedVideoManagerI {
    ...
    private void initI(Context context,
                       ...
                       LMConfigI config) throws IllegalArgumentException {

        LMAdRequest lmAdRequest = (LMAdRequest) request.getInnerIml();
        LMConfig lmConfig = (LMConfig) config.getInnerIml();
        init(context, lmAdRequest, lmConfig);
    }
...
//        adQueueManager.deleteCacheContinuously = config.deleteCacheContinuously;
    ...
    public void prepare(Context context, LMAdRequestI request,
                        LMConfigI config) {
        if (isPrepared) {
            return;
        }
        initI(context.getApplicationContext(), request, config);
        isPrepared = true;
    }
